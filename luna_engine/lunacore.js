//! luna
//! version : 2.6.1
//! author  : sathish kumar @sa-this
//! license : MIT
//! https://www.sa-this.com/luna/
...
(function (root, factory) {
  ...
  if (typeof define === 'function' && define.amd) { // AMD + global
    define([], function () {
      return (root.luna = factory(root));
    });
  } else if (typeof module === 'object' && module.exports) { // CommonJS
    module.exports = factory(root);
  } else { // Browser globals
    root.luna = factory(root);
  }
   ...
   *
   * The quickest way to get started is to visit the [luna homepage](https://www.sa-this.com/luna/).
   *
   * For a more in-depth look at luna, read on.
   *
   * # API Reference
   */
  ...
  var parseResults = function (results) {
    ...
    invokeCallbacks(callbacks.resultNoMatch, results);
      /////////////////////////////////////////////////////////////////////////          
      //run the no command match function
      // nocommandmatch();
      // handleallcommand();
      handleallcommand(window.currentcmd);
      /////////////////////////////////////////////////////////////////////////          
  ...
  luna = {
    /**
     * Add commands that luna will respond to. Similar in syntax to init(), but doesn't remove existing commands.
     *
     * #### Examples:
     * ````javascript
     ...
     * luna.addCommands(commands2);
     * // luna will now listen for all three commands
     * ````
     *
     * @param {Object} commands - Commands that luna should listen for
     * @method addCommands
     ...
     *
     * - `autoRestart`  (boolean) Should luna restart itself if it is closed indirectly, because of silence or window conflicts?
     * - `continuous`   (boolean) Allow forcing continuous mode on or off. luna is pretty smart about this, so only set this if you know what you're doing.
     * - `paused`       (boolean) Start luna in paused mode.
     *
     * #### Examples:
     * ````javascript
     * // Start listening, don't restart automatically
     * luna.start({ autoRestart: false });
     * // Start listening, don't restart automatically, stop recognition after first phrase recognized
     * luna.start({ autoRestart: false, continuous: false });
     * ````
     ...
     *
     * Alternatively, to only temporarily pause luna responding to commands without stopping the SpeechRecognition engine or closing the mic, use pause() instead.
     * @see [pause()](#pause)
     *
     * @method abort
     */
    ...
    /**
     * Pause listening. luna will stop responding to commands (until the resume or start methods are called), without turning off the browser's SpeechRecognition engine or the mic.
     *
     ...
     * @method setLanguage
     * @see [Languages](https://github.com/sa-this/luna/blob/master/docs/FAQ.md#what-languages-are-supported)
     */
     ...
     * luna.removeCommands('hello');
     *
     * // Don't respond to howdy or hi
     * luna.removeCommands(['howdy', 'hi']);
     * ````
     ...
     *
     *     This will fire once per Speech Recognition starting. See https://is.gd/luna_sound_start.
     *
     ...
     *
     * * `resultMatch` - Fired when luna was able to match between what the user said and a registered command.
     *
     ...
     * #### Examples:
     * ````javascript
     * luna.addCallback('error', function() {
     *   $('.myErrorText').text('There was an error!');
     * });
     *
     * luna.addCallback('resultMatch', function(userSaid, commandText, phrases) {
     ...
     * luna.addCallback('errorNetwork', notConnected, this);
     * ````
     ...
     * #### Examples:
     * ````javascript
     * luna.addCallback('start', myFunction1);
     * luna.addCallback('start', myFunction2);
     * luna.addCallback('end', myFunction1);
     * luna.addCallback('end', myFunction2);
     *
     * // Remove all callbacks from all events:
     * luna.removeCallback();
     *
     * // Remove all callbacks attached to end event:
     * luna.removeCallback('end');
     *
     * // Remove myFunction2 from being called on start:
     * luna.removeCallback('start', myFunction2);
     *
     * // Remove myFunction1 from being called on all events:
     * luna.removeCallback(undefined, myFunction1);
     * ````
     *
     ...
     * Returns false if speech recognition is off or luna is paused.
     *
     * @return boolean true = SpeechRecognition is on and luna is listening
     * @method isListening
     */
    ...
    /**
     * Returns the instance of the browser's SpeechRecognition object used by luna.
     * Useful in case you want direct access to the browser's Speech Recognition engine.
     *
     * @returns SpeechRecognition The browser's Speech Recognizer currently used by luna
     ...
     * #### Examples:
     * ````javascript
     * luna.trigger('Time for some thrilling heroics');
     * luna.trigger(
    ...
    trigger: function (sentences) {
      if (!luna.isListening()) {
        if (debugState) {
          if (!isListening) {
            logMessage('Cannot trigger while luna is aborted');
          } else {
            logMessage('Speech heard, but luna is paused');
          }
        }
        return;
      }
    ...
    /**
     * Initialize luna with a list of commands to recognize.
     *
     * #### Examples:
     * ````javascript
     ...
     * const commands2 = {'hi': helloFunction};
     *
     * // initialize luna, overwriting any previously added commands
     * luna.init(commands, true);
     * // adds an additional command without removing the previous commands
     * luna.init(commands2, false);
     * ````
     * As of v1.1.0 it is no longer required to call init(). Just start() listening whenever you want, and addCommands() whenever, and as often as you like.
     *
     * @param {Object} commands - Commands that luna should listen to
    ...
    init: function (commands, resetCommands = true) {
      ...
      recognition.maxAlternatives = 1;  // originally - 5 not 1
      /////////////////////////////////////////////////////////////////////////          
      ...
      // Sets the language to the default 'en-US'. This can be changed with luna.setLanguage()
      ...
      recognition.onend = function () {
        ...
        // luna will auto restart if it is closed automatically and not by user action.
        if (autoRestart) {
          // play nicely with the browser, and never restart luna automatically more than once per second
          var timeSinceLastStart = new Date().getTime() - lastStartedAt;
          autoRestartCount += 1;
          if (autoRestartCount % 10 === 0) {
            if (debugState) {
              logMessage(
                'Speech Recognition is repeatedly stopping and starting. See http://is.gd/luna_restarts for tips.'
              );
            }
          }
          if (timeSinceLastStart < 1000) {
            setTimeout(function () {
              luna.start({ paused: pauseListening });
 ...
 *
 * luna understands commands with `named variables`, `splats`, and `optional words`.
 *
 ...
 * <script>
 * const commands = {
 *   // luna will capture anything after a splat (*) and pass it to the function.
 ...
 *
 *   // By defining a part of the following command as optional, luna will respond
